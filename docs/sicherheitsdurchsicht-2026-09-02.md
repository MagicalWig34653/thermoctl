...
**Empfehlung:** Antwortsignatur, Namespace und erwartete Geräte-/Kanalbezüge prüfen und anschließend den Zustand mit einem signierten `GET` lesen. Ohne verifizierten Zielzustand keinen dauerhaften `EXECUTED`-Cache setzen.
...
### Niedrig — Webhook folgt beliebigen Zielen und nimmt `Authorization` über Hostwechsel mit — **neu**

**Beleg:** Ziel und Token stammen ausschließlich aus der Prozessumgebung (`thermoctl/config.py:64-65`); die Oberfläche zeigt sie nur lesend (`thermoctl/web/control_views.py:236-259`, `thermoctl/domain/interfaces.py:198-220`). `thermoctl/integrations/notification.py:14-31` übergibt die beliebige URL samt Bearer-Header an `urllib.request.urlopen`, das Weiterleitungen folgt. Der Beweis-Test `tests/test_security_review_2026_09_02.py:131-154` zeigt für die verwendete Python-Version, dass ein 302 zu `127.0.0.1` in einen GET umgewandelt und `Authorization` beibehalten wird.
...
**Empfehlung:** Nur `https` erlauben, Weiterleitungen deaktivieren oder auf denselben Ursprung beschränken und private, Loopback-, Link-Local- sowie Metadatenadressen nach DNS-Auflösung sperren. Den `Authorization`-Header nie ursprungsübergreifend weiterreichen.
