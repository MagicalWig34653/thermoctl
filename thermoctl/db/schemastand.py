...
def _kopf_der_migrationen() -> str | None:
...
def stand_der_datenbank(engine: Engine) -> str | None:
    """Die eingetragene Revision, oder None, wenn die Datenbank kein Schema hat."""
    try:
        if not inspect(engine).has_table("alembic_version"):
            return None
        with engine.connect() as verbindung:
            eintrag = verbindung.scalar(text("SELECT version_num FROM alembic_version"))
            return str(eintrag) if eintrag is not None else None
    except SQLAlchemyError:  # pragma: no cover - unerreichbare Datenbank meldet sich selbst
        return None


def schema_pruefen(engine: Engine) -> None:
    ...
    """
    stand = stand_der_datenbank(engine)
    if stand is None:
        if not inspect(engine).has_table("user"):
            raise SchemaPasstNicht(
                f"Die Datenbank hat kein Schema. Vor dem ersten Start einmal '{BEFEHL}' "
    ...
    kopf = _kopf_der_migrationen()
    if kopf is None:
        log.debug("Migrationsverzeichnis nicht gefunden, Versionsvergleich entfaellt")
        return
    if stand != kopf:
        raise SchemaPasstNicht(
            f"Das Datenbankschema steht auf {stand}, der Code erwartet {kopf}. "
            f"'{BEFEHL}' ausfuehren."
        )
