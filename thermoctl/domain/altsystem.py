...
_ZAHL_ATTRIBUTE: Final = frozenset({"temperatureActual", "temperatureTarget"})
...
def beobachtung_aus_topic(topic: str, nutzlast: bytes | str) -> AltsystemBeobachtung | None:
    ...
    try:
        text = nutzlast.decode("utf-8") if isinstance(nutzlast, bytes) else nutzlast
    except UnicodeDecodeError:
        log.warning("Altsystem-Nutzlast ist nicht als UTF-8 lesbar", extra={"topic": topic})
        return None
    text = text.strip()

    if attribut in _ZAHL_ATTRIBUTE:
        try:
            zahl = Decimal(text)
            ...
            return None
        return AltsystemBeobachtung(thermostat_id, attribut, None, zahl)

    if attribut in _TEXT_ATTRIBUTE:
        return AltsystemBeobachtung(thermostat_id, attribut, text, None)
