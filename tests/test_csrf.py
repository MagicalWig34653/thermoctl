...
def test_api_nimmt_kein_sitzungscookie_an(client: TestClient, session, user) -> None:
    """Traegt die Ausnahme oben: Die API laesst sich nicht per Cookie authentifizieren."""
    _entry, geheimnis = create_session(session, user, 3600)
...
def test_aendernde_route_ohne_token_wird_abgewiesen(
    ...
    response = angemeldeter_client.post(pfad)
...
def test_aendernde_route_mit_gueltigem_token_laeuft_durch(
    ...
    response = angemeldeter_client.post("/logout", headers=kopf, follow_redirects=False)
...
def test_csrf_cookie_wird_bei_der_anmeldung_gesetzt(client: TestClient, user) -> None:
