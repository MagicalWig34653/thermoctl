...
def test_get_session_committet_bei_erfolg(engine: Engine) -> None:
    ...
    sitzung = next(erzeuger)
    sitzung.execute(text("SELECT 1"))
...
def test_geschuetzte_seite_bei_inaktivem_benutzer_ist_401(
    ...
    _sitzung, geheimnis = sitzung_anlegen(session, benutzer, 3600)
    session.flush()
    client.cookies.set(COOKIE_NAME, geheimnis)
    assert client.get("/benutzer").status_code != 401, "Vorbedingung: angemeldet"
...
def test_cli_reicht_die_einstellungen_an_uvicorn_durch(
    ...
    with patch("uvicorn.run") as gestartet:
        main()
    get_settings.cache_clear()

    gestartet.assert_called_once()
    _args, kwargs = gestartet.call_args
