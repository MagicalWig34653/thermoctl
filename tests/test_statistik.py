...
def _protokoll(session: Session, zone_id: int, muster: list[tuple[int, bool]]) -> None:
    """`muster` ist eine Folge aus (Minuten seit BEGINN, wuerde heizen)."""
    from decimal import Decimal

    for versatz, heizt in muster:
        session.add(
            ShadowDecision(
...
def test_dauer_kommt_aus_den_abstaenden(session: Session) -> None:
    ...
    zone = zone_anlegen(session, "statistikzone")
    _protokoll(session, zone.id, [(0, True), (1, True), (2, False)])

    ergebnis = heizzeiten(
        session, [zone.id], BEGINN, BEGINN + timedelta(hours=1), zyklus_sekunden=60
    )
    assert ergebnis[zone.id].sekunden_gesamt == 120
...
def test_der_letzte_messpunkt_zaehlt_nicht_ins_unendliche(session: Session) -> None:
    ...
    beitragen."""
    zone = zone_anlegen(session, "letzterpunkt")
    _protokoll(session, zone.id, [(0, True)])
    ergebnis = heizzeiten(
        session, [zone.id], BEGINN, BEGINN + timedelta(hours=1), zyklus_sekunden=60
    )
    assert ergebnis[zone.id].sekunden_gesamt == 0
...
def test_eine_luecke_wird_gekappt(session: Session) -> None:
    ...
    zone = zone_anlegen(session, "luecke")
    _protokoll(session, zone.id, [(0, True), (480, True), (481, False)])
    ergebnis = heizzeiten(
        session, [zone.id], BEGINN, BEGINN + timedelta(days=1), zyklus_sekunden=60
    )
    # 3 Minuten Kappung fuer die Luecke, dazu die eine echte Minute danach.
    assert ergebnis[zone.id].sekunden_gesamt == 180 + 60
...
def test_ohne_kappung_waere_es_ein_ganzer_arbeitstag(session: Session) -> None:
    ...
    zone = zone_anlegen(session, "ohnekappung")
    _protokoll(session, zone.id, [(0, True), (480, True), (481, False)])
    ergebnis = heizzeiten(
        session, [zone.id], BEGINN, BEGINN + timedelta(days=1), zyklus_sekunden=100000
    )
    assert ergebnis[zone.id].sekunden_gesamt == 480 * 60 + 60
...
def test_ein_langsamerer_zyklus_zaehlt_richtig(session: Session) -> None:
    ...
    zone = zone_anlegen(session, "langsam")
    _protokoll(session, zone.id, [(0, True), (5, True), (10, False)])
    ergebnis = heizzeiten(
        session, [zone.id], BEGINN, BEGINN + timedelta(hours=1), zyklus_sekunden=300
    )
    assert ergebnis[zone.id].sekunden_gesamt == 600


def test_tage_werden_getrennt(session: Session) -> None:
    zone = zone_anlegen(session, "tagesgrenze")
    _protokoll(
        session,
        zone.id,
    ...
    ergebnis = heizzeiten(
        session, [zone.id], BEGINN, BEGINN + timedelta(days=2), zyklus_sekunden=60
    )
    nach_tag = {t.tag: t.sekunden for t in ergebnis[zone.id].tage}
    assert nach_tag[BEGINN.date()] == 60
    assert nach_tag[(BEGINN + timedelta(days=1)).date()] == 60
...
def test_zone_ohne_protokoll_kommt_mit_nullen_vor(session: Session) -> None:
    ...
    zone = zone_anlegen(session, "stille-zone")
    ergebnis = heizzeiten(
        session, [zone.id], BEGINN, BEGINN + timedelta(days=2), zyklus_sekunden=60
    )
    assert zone.id in ergebnis
    assert len(ergebnis[zone.id].tage) == 3
    assert ergebnis[zone.id].sekunden_gesamt == 0


def test_dauer_in_worten() -> None:
    assert als_dauer(0) == "–"
    assert als_dauer(59) == "1m"
    assert als_dauer(35 * 60) == "35m"
    assert als_dauer(4 * 3600 + 5 * 60) == "4h 05m"
